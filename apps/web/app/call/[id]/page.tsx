...
function groupBySpeaker(transcript: TranscriptUtterance[]): {
  speaker: TranscriptUtterance["speaker"];
  ts: number;
  text: string;
}[] {
  const groups: { speaker: TranscriptUtterance["speaker"]; ts: number; text: string }[] = [];
  for (const u of transcript) {
    const last = groups[groups.length - 1];
    if (last && last.speaker === u.speaker) {
      last.text += " " + u.text.trim();
    } else {
      groups.push({ speaker: u.speaker, ts: u.ts_start, text: u.text.trim() });
    }
  }
  return groups;
}

function buildTranscriptText(transcript: TranscriptUtterance[], callId: string): string {
  if (transcript.length === 0) return "(no utterances captured)\n";

  // Group consecutive utterances from the same speaker into a single paragraph
  // so the downloaded file reads like a real transcript instead of jagged lines.
  const groups = groupBySpeaker(transcript);

  const wrap = (line: string, width = 90): string => {
  ...
  const header = [
    "RevAgent — Discovery Call Transcript",
    `Call ID:    ${callId}`,
    `Exported:   ${new Date().toISOString()}`,
    `Utterances: ${transcript.length}  ·  Speakers: ${new Set(transcript.map((u) => u.speaker)).size}`,
    "─".repeat(60),
    "",
  ].join("\n");

  const body = groups
    .map((g) => `[${fmtTimestamp(g.ts)}]  Speaker ${g.speaker}\n${wrap(g.text)}\n`)
    .join("\n");

  const footer = [
    "─".repeat(60),
    "Generated by RevAgent · revagent.vercel.app",
    "",
  ].join("\n");
...
function TranscriptCard({
  ...
  async function copyAll() {
    try {
      await navigator.clipboard.writeText(buildTranscriptText(transcript, callId));
      setCopied(true);
      setTimeout(() => setCopied(false), 1500);
    } catch {
      /* clipboard blocked */
  ...
  function download() {
    const blob = new Blob([buildTranscriptText(transcript, callId)], {
  ...
  return (
    <Card className="rise-in">
      ...
      <div
        className="mt-4 max-h-80 overflow-y-auto rounded-xl bg-white/70 p-4 text-sm leading-relaxed text-navy scrollbar-thin space-y-3"
        aria-label="Call transcript"
      >
        {groupBySpeaker(transcript).map((g, i) => (
          <div key={i} className="space-y-1">
            <p className="font-mono text-[10px] uppercase tracking-wider text-blue-700 font-bold">
              [{fmtTimestamp(g.ts)}] Speaker {g.speaker}
            </p>
